...
[ -z "$(git status --porcelain --untracked-files=no)" ] || { echo 'Error: commit tracked worktree changes first' >&2; exit 1; }
...
git grep -Fq 'ref: ${{ inputs.version }}' HEAD -- .github/workflows/release.yml \
  || { echo 'Error: current commit lacks the tag-based release workflow' >&2; exit 1; }
...
git grep -Fq 'ref: ${{ inputs.version }}' "origin/$WORKFLOW_BRANCH" -- .github/workflows/release.yml \
  || { echo 'Error: updated release workflow is not on the remote workflow branch' >&2; exit 1; }
old_tag="${remote_tag%%$'\t'*}"
