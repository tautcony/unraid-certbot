<?php
/**
 * unraid-certbot 本地预览：.page 渲染所需的仿真逻辑。
 *
 * Unraid 的 webGUI 会先解析 .page 文件头部的 `Key="Value"` 指令，再把 `---`
 * 之后的正文当 PHP 执行，最后套上主题外壳。这里按同样的顺序做一遍，
 * 好让开发机上的浏览器看到接近真机的设置页 / 状态页。
 *
 * 外壳（#header / #menu / div.title / #footer）以及 dl、表格、按钮、输入框的样式
 * 抄自 Unraid 7 的 webGUI 源码：
 *   emhttp/plugins/dynamix/styles/default-base.css
 *   emhttp/plugins/dynamix/styles/default-color-palette.css
 *   emhttp/plugins/dynamix/styles/themes/{black,white}.css
 * 只保留预览用得到的那部分，具体差异见 dev/README.md。
 *
 * 只服务于本地调试，不参与插件打包。
 */

/** Unraid 官方 wordmark 路径（来自 webGui 的 Header.php） */
...
/** 把正文当 PHP 执行，返回渲染结果；出错时把异常画在页面上，方便定位 */
function cb_dev_eval_body(string $body): string
{
    // .page 正文里会直接使用 $docroot，Unraid 也是这样把它放进作用域的
    global $docroot;
...
/**
 * 把 Unraid 设置页的「定义列表」写法
 *
 *   _(标签)_:
 *   : <input ...>
 *   : <blockquote class="inline_help">说明</blockquote>
 *
 * 转成 Unraid 解析器（php-markdown Extra）同样的结构：一个 <dl> 里一个 <dt>
 * 跟着若干 <dd>。说明性文字写在 <blockquote> 里且不缩进，所以这里用
 * 「未闭合块级标签数」判断续行，而不是只看缩进。
 */
...
function cb_dev_render_page(string $file, array $ctx = []): string
{
    ...
    // 先按定义列表切块（此时 _(...)_ 标记还在，用来识别标签行），再统一翻译
...
/**
 * Unraid 顶部菜单。只有预览里真实存在的页面可点，其余保持灰显，
 * 目的是让外壳看起来和真机一致，而不是做一个假菜单。
 *
 * 菜单文字按中文语言包写，和真机中文界面对得上。
 */
...
/** 把页面正文套进 Unraid 外壳，并注入预览用的弹窗 / 表单脚本 */
function cb_dev_chrome(array $head, string $body, array $ctx = []): string
{
    ...
    // ?theme=black|white 一次性指定主题；默认跟真机截图一致的 white（深色顶栏 + 浅色正文）
...
<style>
/* ===================================================================
   Unraid 7 官方配色（default-color-palette.css）
   =================================================================== */
:root{
...
/* Unraid 7 themes/black.css —— 深色正文 + 浅色顶栏 */
...
/* Unraid 7 themes/white.css —— 浅色正文 + 深色顶栏 */
...
/* ===================================================================
   Unraid 7 default-base.css（只取预览用得到的那部分）
   =================================================================== */
...
/* 说明块：Unraid 7 默认隐藏，点标签或顶部 HELP 展开 */
...
/* 页脚：窄屏跟随文档流，桌面宽度固定贴在视口底部（与 Unraid 一致） */
...
/* ===================================================================
   让插件 .page 自带的内联样式跟随 Unraid 主题变量。
   这段放在正文之后，靠顺序覆盖，不需要 !important。
   =================================================================== */
...
<script>
/* 主题切换：沿用 Unraid 的 black / white 两套变量 */
(function () {
...
/* 说明文字：Unraid 7 默认隐藏 blockquote.inline_help，点标签或 HELP 展开 */
...
/* 对应 Unraid 的 openBox()：带 iframe 的模态框 */
...
/* 设置保存完成后由 iframe 回调，把进度框留在界面上方便看输出 */
